...
int main(int argc, char** argv)
{
    ...
    bool skip_postprocess = (std::getenv("SKIP_POSTPROCESS") != nullptr);
    bool skip_draw = headless || (std::getenv("SKIP_DRAW") != nullptr);
    int warmup_frames = 10;

    auto yolov8 = new YOLOv8(engine_file_path);
    yolov8->make_pipe(true);
    if (isVideo && input_arg == "camera") {
        printf("E2E mode: headless=%d, skip_postprocess=%d, skip_draw=%d\n",
               headless ? 1 : 0, skip_postprocess ? 1 : 0, skip_draw ? 1 : 0);
    }
    if (profile_enabled) {
    ...
    if (isVideo) {
        cv::VideoCapture cap;
        if (input_arg == "camera") {
            // Dùng cấu hình camera giống usb_cam_pro.py
        ...
        if (!cap.isOpened()) {
            if (input_arg == "camera") {
                printf("can not open camera (video0, V4L2)\n");
            } else {
        ...
        auto   cam_t0          = std::chrono::steady_clock::now();
        int    cam_frames      = 0;
        double cam_fps_display = 0.0;
        auto   infer_t0        = std::chrono::steady_clock::now();
        std::deque<double> infer_ms_window;
        int    infer_seen    = 0;
        ...
        while (true) {
            ...
            auto tc = (double)std::chrono::duration_cast<std::chrono::microseconds>(t_inf1 - t_inf0).count() / 1000.;

            infer_seen++;
            if (infer_seen > warmup_frames) {
                infer_ms_window.push_back(tc);
                if (infer_ms_window.size() > 30) infer_ms_window.pop_front();
                if (profile_enabled) {
                    read_ms_sum +=
            ...
            auto cam_elapsed_ms =
                std::chrono::duration_cast<std::chrono::milliseconds>(now - cam_t0).count();
            if (cam_elapsed_ms >= 1000) {
                cam_fps_display = cam_frames * 1000.0 / cam_elapsed_ms;
                cam_frames = 0;
                cam_t0 = now;
            }

            auto infer_elapsed_ms =
                std::chrono::duration_cast<std::chrono::milliseconds>(now - infer_t0).count();
            if (infer_elapsed_ms >= 1000) {
                double infer_avg_ms = 0.0;
                if (!infer_ms_window.empty()) {
                    double sum = 0.0;
                    for (double v : infer_ms_window) sum += v;
                    infer_avg_ms = sum / infer_ms_window.size();
                }
                double infer_fps = infer_avg_ms > 0.0 ? 1000.0 / infer_avg_ms : 0.0;
                printf("Camera FPS: %.1f | Infer FPS: %.1f | Infer avg: %.1f ms\n",
                       cam_fps_display, infer_fps, infer_avg_ms);
                infer_t0 = now;
            }
